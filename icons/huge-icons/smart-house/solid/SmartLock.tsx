import * as React from "react";
import type { SVGProps } from "react";
const SvgSmartLock = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    {...props}
  >
    <path
      fill="currentColor"
      fillRule="evenodd"
      d="M8.75 6a3.25 3.25 0 0 1 6.5 0v2h-6.5zM7.253 8.07 7.25 8V6a4.75 4.75 0 0 1 9.5 0v2l-.003.07A4 4 0 0 1 20 12v6a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4v-6a4 4 0 0 1 3.253-3.93M12.75 18a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0m-4.462-3.712A5.23 5.23 0 0 1 12 12.75c1.45 0 2.761.587 3.712 1.538a.75.75 0 1 0 1.06-1.061A6.73 6.73 0 0 0 12 11.25a6.73 6.73 0 0 0-4.773 1.977.75.75 0 0 0 1.06 1.06m2.121 2.121A2.24 2.24 0 0 1 12 15.75a2.24 2.24 0 0 1 1.591.659.75.75 0 1 0 1.06-1.06A3.74 3.74 0 0 0 12 14.25a3.74 3.74 0 0 0-2.652 1.098.75.75 0 0 0 1.061 1.061"
      clipRule="evenodd"
    />
  </svg>
);
export default SvgSmartLock;
